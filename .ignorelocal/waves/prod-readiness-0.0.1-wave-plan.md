...
**Status:** Batch F W23 done 2026-08-06 (D54 last impl wave); W21.5–W21.8 un-xfail done 2026-08-06; F-Verify / F-Final / F-Thermos / F-Reverify done; F-PR pending
**Date:** 2026-08-05
...
| W1 | **test-creator** | — | RED: settings allowlist, chain resolution both sides, no env fallbacks, loud client failure, bootstrap file, preflight, authenticated healthcheck | [ ] |
| W2 | executor | C3.1, C1.5 | One configuration authority — `ProcessSettings` + gateway-side chain resolution | [ ] |
| W3 | executor | C3.2, C3.3 | Thread the secret; delete eleven `os.environ` reads **and** the write-back (D41); fail loudly on empty | [ ] |
| W4 | executor | C1.2 | Generate the secret into `sevn-state` from a one-shot init service (D37) | [ ] |
| W5 | executor | C1.3, C1.4 | `compose-up` preflight over three variables (D38); authenticated proxy healthcheck (D39) | [ ] |
| A-Verify / A-Final / A-Thermos / A-Reverify | verifier / executor / review / **fresh verifier** | — | Batch gates (D29, D30, D40) | [ ] / [ ] / [ ] / [ ] |
...
| W13 | **test-creator** | C9.3 | RED + prerequisite: unpin the full-tree `find` (D48); overlay-wide `--no-sandbox` guard; marker; `HostConfig` limits | [ ] |
| W14 | executor | C8.1, C8.2, C8.4 | Remove `--no-sandbox` from prod; widen guard to overlays; delete stale comments; site-isolation decision | [ ] |
| W15 | executor | C9.1, C9.2, C9.4 | **After A merges (D35)** — scoped chown, versioned init marker, `sevn-ci-init` parity | [ ] |
| W16 | executor | C10.1, C10.2, C10.3 | Compose version floor; `HostConfig` integration check; limits where resolved config lacks them (D49) | [ ] |
| W17 | executor | C8.3 | Browser as its own minimally-privileged service (D50 — split to a follow-up PR if it destabilises the batch) | [ ] |
| D-Verify / D-Final / D-Thermos / D-Reverify | verifier / executor / review / **fresh verifier** | — | Batch gates (D29, D30) | [ ] / [ ] / [ ] / [ ] |
...
| W21 | **test-creator** | — | RED: trust-address refused under tunnel/proxy, boot warning, CLI message, driver coverage, tag-path exit-2 handling | [x] (2026-08-05 ✅: `3001d6d5` — RED suite + guards; 18 pass / 14 xfail) |
| W22 | executor | C6.2, C6.4 | Guard and rename the trust-address escape; warn at boot; fix the CLI message | [x] (2026-08-05 ✅: `a4e660b6` — refuse under tunnel/non-loopback; boot warning; keep name; CLI honesty) |
| W23 | executor | C14.1, C14.2, C14.3 | Wire `make verify-deployment` into cron + tag path (D52); new drivers; attach evidence to the release | [x] (2026-08-06 ✅: c432710b — cron+tag verify-deployment jobs, 4+1 DRIVERS registered, `evidence/verify/*.json` upload + phase6 attachment, spec 25 in-place prose + Workflow matrix + Failure Modes, drift sweep across 7 about-docs slugs + 3 readme slugs) |
| F-Verify / F-Final / F-Thermos / F-Reverify | verifier / executor / review / **fresh verifier** | — | Batch gates (D29, D30) | [x] (2026-08-07 ✅: `f2d04007` — F-V5 un-xfail landed; 9/9 drivers green on clean run; 14/14 W21 RED tests pass for real; C6.1 unmodified suite 15/15; `make ci-affected` exit 0; P5 clean) / [x] (2026-08-07 ✅: `e5a5ee47` — all 41 ci-resume steps green; 0 xfails over Batch F RED (50 passed); FF-V1 regression from `c432710b` resolved; FF-V2 (h2 CVE-2026-71554 allowlist, fix already on `origin/pre-0.0.1` via PR #242) and FF-V3 (spec-25 fingerprint refresh) added in this re-dispatch) / [x] (2026-08-07 ✅: `57821c96` — clean including low; 7 original findings all resolved; rebase intact; P2/P3/P5 clean) / [x] (2026-08-07 ✅: `978ea105` — fresh verifier per D30; all 5 proofs re-confirmed; P4a holds for F-Thermos fixes; P2/P3/P5 clean) |
...
- [ ] **W1.1** Assert `SEVN_PROXY_SHARED_SECRET` is a member of `PROCESS_SETTINGS_ENV_VAR_NAMES` (`src/sevn/config/settings.py:28-38`) and parses into `ProcessSettings` (**xfail → W2**). New `tests/config/test_prod_ready_proxy_secret_settings_w1_red.py`.
- [ ] **W1.2** Assert the **gateway** side resolves the secret from the secrets chain, not only the proxy process — `src/sevn/agent/adapters/egress_bridge.py:55` is env-only today (**xfail → W2**).
- [ ] **W1.3** Assert **zero** `os.environ.get("SEVN_PROXY_SHARED_SECRET")` reads remain under `src/` outside the single resolution seam, and that `src/sevn/proxy/credentials.py` no longer **writes** `os.environ` (D41) (**xfail → W3**). A source-level assertion is correct here: the defect is the *existence* of the fallbacks.
- [ ] **W1.4** Assert a guarded-route client with an empty resolved secret raises a named, actionable error rather than sending an empty header (**xfail → W3**) — cover `tools/web.py`, `tools/integration_proxy_client.py`, `integrations/proxy_client.py`, `integrations/github_skill/hooks.py`.
- [ ] **W1.5** Assert the bootstrap contract (**xfail → W4**): on a clean `sevn-state` volume a secret file is created at the agreed path with mode `0600` and uid `10001`; it is **not** regenerated on the next boot; an explicitly-set env value takes precedence; `.env.example` contains **no** blank `SEVN_PROXY_SHARED_SECRET=` line (D37).
- [ ] **W1.6** Assert the preflight (**xfail → W5**, D38) rejects empty, below-entropy-threshold, and placeholder values (`change-me`) for `SEVN_PROXY_SHARED_SECRET`, `SEVN_GATEWAY_TOKEN` and `SEVN_SECRETS_PASSPHRASE`, and that it runs **before** services start.
- [ ] **W1.7** Assert the proxy healthcheck presents the service secret against a guarded prefix and that **503/401 is unhealthy** (**xfail → W5**, D39). Parse `docker/docker-compose.yml` directly so the test runs without a Docker daemon.
- [ ] **W1.8** **Guard test (D40):** assert `tests/proxy/test_auth.py` and `tests/proxy/test_post_audit_proxy_auth_w4_red.py` pass **unmodified** — the 503 branch, the `SEVN_PROXY_ALLOW_UNAUTHENTICATED` opt-in, and the boot warning are landed contracts.
- [ ] **W1.9** Author `docs/test-plans/prod-ready-batch-a.md`; `make lint typecheck` on touched paths; commit + push.
...
- [ ] **W2.1** Add `SEVN_PROXY_SHARED_SECRET` to `PROCESS_SETTINGS_ENV_VAR_NAMES` (`src/sevn/config/settings.py:28-38`) and the corresponding `ProcessSettings` field, so the variable is inside the audited configuration contract rather than beside it.
- [ ] **W2.2** Give the **gateway** the same secrets-chain resolution the proxy already has: `resolve_proxy_shared_secret` (`src/sevn/agent/adapters/egress_bridge.py:55`) consults the workspace secrets chain before falling back to process env, mirroring `_resolve_proxy_shared_secret` (`src/sevn/proxy/credentials.py:810`). One resolution seam, two processes.
- [ ] **W2.3** Confirm `ProxySettings.proxy_shared_secret` (`src/sevn/proxy/settings.py:40-43`) keeps its `AliasChoices` env binding — the env path stays supported for external secret managers; it stops being the *only* path.
- [ ] **W2.4** Run **`make config-schema`** and update `infra/sevn.schema.json` env documentation for the variable. Add the Amendments to specs 02 and 06 (D53).
- [ ] **W2.5** Un-xfail W1.1/W1.2; `make lint typecheck`; commit + push — *(suggested: `fix(config): make the proxy shared secret a first-class settings surface`)*.
...
- [ ] **W3.1** Thread the resolved secret into `build_runtime_tool_bindings` from the gateway (`src/sevn/gateway/http_server.py` — the bindings call site currently passes `proxy_url` and `session_token` but no shared secret), so tool clients receive it by injection.
- [ ] **W3.2** Delete the `os.environ.get("SEVN_PROXY_SHARED_SECRET")` reads at the ten call sites from W0.5, replacing each with the injected value. Where a site is genuinely out-of-process (`data/bundled_skills/core/job-ops/scripts/lib/llm.py:87` runs inside a sandbox), keep the env read but document it as the sandbox child-env contract rather than a gateway fallback — and record that decision in the wave notes.
- [ ] **W3.3** **In the same commit**, delete the `os.environ["SEVN_PROXY_SHARED_SECRET"] = proxy_secret` write-back at `src/sevn/proxy/credentials.py:805-806` (D41). Verify the proxy still resolves its secret through `settings.model_copy(update=…)` alone.
- [ ] **W3.4** **C3.3** — make a guarded-route client with an empty resolved secret raise a named, actionable error at call time instead of sending an empty header and surfacing an opaque 401. Name the variable and the remedy in the message.
- [ ] **W3.5** Add the Amendment to spec 07 describing the single configuration authority and the client-side failure mode (D53).
- [ ] **W3.6** Un-xfail W1.3/W1.4; `make lint typecheck`; commit + push — *(suggested: `fix(proxy): resolve the shared secret through one authority`)*.
...
- [ ] **W4.1** Add a one-shot init step that generates a high-entropy secret into the shared `sevn-state` volume (e.g. `/operator/.sevn/proxy-shared-secret`, mode `0600`, owner `10001:10001`) **only when absent**. Reuse `sevn-operator-perms` (`docker/docker-compose.yml:50-63`) or add a sibling with `restart: "no"` — record which and why.
- [ ] **W4.2** Have **both** `sevn-proxy` (`:76`) and `sevn-gateway` (`:108`) read the generated value, with an explicitly-set `SEVN_PROXY_SHARED_SECRET` still taking precedence. Apply the same to `docker-compose.browser.yml:18` and `docker-compose.gui.yml:18`.
- [ ] **W4.3** **Remove** the blank `SEVN_PROXY_SHARED_SECRET=` line from `.env.example:65` and rewrite the comment at `:64` to describe generation. Do not replace it with another empty placeholder (D37).
- [ ] **W4.4** Wire the same generation into the onboarding path so a host install and a Compose install converge on one secret location. Confirm the chain resolution from W2.2 finds it.
- [ ] **W4.5** Update `docker/README.md:82` — which currently instructs the operator to set the variable on both services by hand — to document the generated default and the override.
- [ ] **W4.6** Add the Amendments to specs 22 and 06; un-xfail W1.5; `make lint typecheck`; commit + push — *(suggested: `fix(compose): generate the proxy shared secret on first boot`)*.
...
- [ ] **W5.1** Add the preflight (D38): reject an empty value, a value below a minimum entropy threshold, and a placeholder blacklist that includes **`change-me`**, for `SEVN_PROXY_SHARED_SECRET`, `SEVN_GATEWAY_TOKEN` and `SEVN_SECRETS_PASSPHRASE`. Fail **before** services start.
- [ ] **W5.2** Wire it into `make compose-up` (and the browser/GUI wrappers via the shared `COMPOSE_FILES` variable) **and** into a `ci-*` tier, keeping `tests/infra/test_ci_steps_tier_parity.py` green (Global convention 9).
- [ ] **W5.3** Decide and record how the preflight interacts with the W4 generated secret: generation should satisfy it automatically, so the gate fires only when an operator supplies a bad explicit value. A preflight that fails the happy path is a worse defect than the one it fixes.
- [ ] **W5.4** **C1.4** — add the authenticated proxy healthcheck (D39): present the resolved service secret against a guarded prefix; treat 503/401 as unhealthy. Keep `/healthz` (`docker/docker-compose.yml:86-97`) as the liveness probe. Ensure the probe consumes no provider quota — record which endpoint was chosen and why.
- [ ] **W5.5** Update `.env.example` comments for the three variables and `docker/README.md`; add the Amendment to spec 07 and the in-place edit to `25-cicd-full.md` for the new tier check (D53).
- [ ] **W5.6** Un-xfail W1.6/W1.7; `make lint typecheck`; commit + push — *(suggested: `fix(compose): preflight operator secrets and probe authenticated egress`)*.
...
- [ ] **W13.1** **C9.3** — update `tests/infra/test_post_audit_compose_w1_red.py`, which pins the full-tree `find /operator ! -user 10001 …` form, to assert the **new** contract: scoped directories plus a versioned marker. This must land **before** W15 (D48).
- [ ] **W13.2** Assert **no compose file or overlay** passes `--no-sandbox` (**xfail → W14**, C8.1) — today `docker/docker-compose.prod.yml:19`. Extend the existing Dockerfile-scoped guard (`tests/infra/test_release_audit_containers_w7_red.py:68-71`), which does not inspect overlays.
- [ ] **W13.3** Assert the stale `--no-sandbox` comments are gone from `docker/docker-compose.browser.yml:13` and `docker/docker-compose.gui.yml:13` (**xfail → W14**, C8.2).
- [ ] **W13.4** Assert the site-isolation decision is recorded — either `--disable-features=IsolateOrigins,site-per-process` is gone from the login-grade args or a documented justification exists (**xfail → W14**, C8.4).
- [ ] **W13.5** Assert the permissions init writes a **versioned marker** and skips the broad migration when present (**xfail → W15**, C9.2), and that `docker/docker-compose.ci.yml:20` no longer runs an unconditional `chown -R` (**xfail → W15**, C9.4).
...
- [ ] **W13.10** Author `docs/test-plans/prod-ready-batch-d.md`; `make lint typecheck`; commit + push.
...
- [ ] **W14.1** **C8.1** — remove `--no-sandbox` from `docker/docker-compose.prod.yml:19`. The flag is absent from dev and present in **prod** today, which is the inverse of a safe default. Confirm the prod browser/GUI stack still starts without it; if it cannot, record the blocking reason and the alternative rather than restoring the flag silently.
- [ ] **W14.2** Extend the guard to **every compose file and overlay**, not just Dockerfiles — the existing test inspects the image, which is why the overlay escaped.
- [ ] **W14.3** **C8.2** — delete the stale `--no-sandbox` comments at `docker/docker-compose.browser.yml:13` and `docker/docker-compose.gui.yml:13`; both now say Brave runs with a flag the file does not set.
- [ ] **W14.4** **C8.4** — re-justify or drop `--disable-features=IsolateOrigins,site-per-process` from the login-grade Chrome args (`src/sevn/browser/chrome.py`). If it stays for a login-grade session, document the threat model and confirm it is **not** applied to untrusted browsing.
- [ ] **W14.5** Document in `docs/readmes/security.md` that container hardening (`cap_drop: ALL`, `no-new-privileges`) does **not** substitute for the renderer sandbox; add the Amendment to spec 09 (D53).
- [ ] **W14.6** Un-xfail W13.2–W13.4; `make lint typecheck`; commit + push — *(suggested: `fix(compose): stop disabling the browser sandbox in production`)*.
...
- [ ] **W15.1** **D35 gate — do this after Batch A merges.** `git fetch origin && git rebase origin/pre-0.0.1`. If A has not merged, **stop here and report** — do not restructure the file, and in particular do not collide with A's bootstrap init service (W4.1).
- [ ] **W15.2** **C9.1** — replace the full-tree `find /operator ! -user 10001 …` (`docker/docker-compose.yml:60-61`) with chown over the **known application-owned directories** only. Stop walking operator data. Keep `/browser-profiles` (`:62`) equivalently scoped.
- [ ] **W15.3** **C9.2** — write a versioned init marker (e.g. `/operator/.sevn/perms-v1`) and run the broad migration **only** when the marker is absent or stale. Note that `restart: "no"` means the init re-runs on every fresh `up`, which is exactly why the marker matters.
- [ ] **W15.4** **C9.4** — apply the same treatment to `sevn-ci-init` (`docker/docker-compose.ci.yml:20`), which still runs the unconditional `chown -R 10001:10001 /operator` the base file abandoned, **or** document why CI diverges. Divergence that nobody chose is the defect.
- [ ] **W15.5** Coordinate with A's bootstrap service (W4.1): the generated secret file must be owned correctly by whichever init runs first. Record the ordering.
- [ ] **W15.6** Update `docker/README.md`; un-xfail W13.5; `make lint typecheck`; commit + push — *(suggested: `fix(compose): scope the permissions init and gate it on a marker`)*.

**Acceptance:** W13.1 and W13.5 green; a pre-owned volume completes the init in well under the 5s `SEVN_VERIFY_PERMS_MAX_S` budget with no full-tree walk; a fresh volume still yields a healthy stack; `make ci-affected SEVN_CI_BASE=HEAD` green.
...
- [ ] **W16.1** **C10.1** — pin and document a minimum Docker Compose version, and enforce it in the preflight path (`scripts/check-compose-default.sh` or the W5 sibling). `deploy.resources` is honoured by `docker compose` v2 but not by every older `docker-compose`; today no minimum is documented anywhere.
- [ ] **W16.2** **C10.3** — start from the W0.7 `docker compose … config` capture (D49). Add `deploy.resources.limits` and `pids_limit` **only** where the *resolved* configuration lacks them. `docker/docker-compose.ci.yml` is the known gap; the browser and GUI overrides are expected to inherit through the base `sevn-gateway` service and must be verified, not assumed.
- [ ] **W16.3** **C10.2** — add an integration check that creates the containers and reads `HostConfig`, asserting `NanoCpus`, `Memory` and `PidsLimit` are non-zero **and match the declared values**. Today no test or workflow inspects runtime enforcement — only YAML.
- [ ] **W16.4** Wire the check into a `ci-*` tier or the `verify-*` driver family (coordinate with F/W23 so it is not wired twice), keeping `tests/infra/test_ci_steps_tier_parity.py` green.
- [ ] **W16.5** Update `docker/README.md` and `25-cicd-full.md` in place (D53); un-xfail W13.6–W13.8; `make lint typecheck`; commit + push — *(suggested: `fix(compose): prove resource limits are enforced at runtime`)*.

**Acceptance:** W13.6–W13.8 green; the `HostConfig` check fails when a limit is removed from YAML; every resolved file set declares limits for every service; `make ci-affected SEVN_CI_BASE=HEAD` green.
...
- [ ] **W17.5** **D50 escape hatch:** if this wave cannot complete without destabilising the batch, **stop, record the state, and move it to its own follow-up PR** against `pre-0.0.1`. C8.1/C8.2/C8.4 and all of C9/C10 must not be held hostage to it.
- [ ] **W17.6** Add the Amendment to spec 09; un-xfail W13.9; `make lint typecheck`; commit + push — *(suggested: `fix(browser): run the browser as its own minimally-privileged service`)*.

**Acceptance:** W13.9 green **or** the D50 deferral recorded with a follow-up issue; browser skills still function against the split service; `make ci-affected SEVN_CI_BASE=HEAD` green.

### Batch D gates

- [ ] **D-Verify** (`wave-verifier`): **`make verify-compose-profiles`** and **`make verify-stack-health`** on a clean `sevn-state` volume — paste each `VERIFY_OVERALL:`. Runtime proof: `docker inspect` each running container and show `NanoCpus`, `Memory`, `PidsLimit` non-zero and matching (C10.2); show the perms init skipping a pre-owned volume via the marker and completing in well under budget; bring up the prod overlay and show **no** `--no-sandbox` in the browser process args; if W17 landed, show the browser service without a `sevn-state` mount and without `SEVN_GATEWAY_TOKEN`.
- [ ] **D-Final:** xfail sweep (0 xfails); `graphify update .`; **drift sweep in one commit** (D7) covering `docs/readmes/security.md` and `make about-docs-extract` for specs 09 and 25; CHANGELOG entries for C8.\*, C9.\*, C10.\*; `make ci-resume SEVN_CI_BASE=origin/pre-0.0.1` until all steps pass.
- [ ] **D-Thermos:** thermo-nuclear review; **clean including `low`** (D31); **blocks the PR**. Pay specific attention to any hardening removed "to make the stack boot". **Record the base SHA** to `.ignorelocal/waves/prod-ready-d-thermos-base.sha` and **declare every file the review changed** (D29).
- [ ] **D-Reverify** (**fresh `wave-verifier`**): convention-11 detection with `<batch>` = `d`. `docker/docker-compose.yml` and `scripts/check-compose-default.sh` are the two files a reviewer rewrote last time in the predecessor program (`53eff5d7`) — a Thermos edit to either invalidates every D-Verify invocation proof. Re-run the D-Verify proofs in full plus `make ci-affected SEVN_CI_BASE="$BASE"`. Hand findings back (D4). **Blocks D-PR** (D29, D30).
...
- [x] **W21.1** Assert `local_open_trust_address` is **refused** whenever a tunnel or a reverse proxy is configured, even when the key is `true` (**xfail → W22**, C6.2). (2026-08-05 ✅: `3001d6d5` — trust-address force-off xfails)
- [x] **W21.2** Assert a **boot warning** is logged when the trust-address escape is enabled (**xfail → W22**, C6.2), mirroring the `SEVN_PROXY_ALLOW_UNAUTHENTICATED` boot warning pattern already in `src/sevn/proxy/auth.py:76`. (2026-08-05 ✅: `3001d6d5` — boot warning symbol xfails)
- [x] **W21.3** Assert the CLI no longer prints `"loopback access — no login required"` (`src/sevn/cli/commands/dashboard_cmd.py:190`) when a token **is** required (**xfail → W22**, C6.4). (2026-08-05 ✅: `3001d6d5` — CLI no-login-required xfail)
- [x] **W21.4** **Guard test:** assert the landed `C6.1`/`C6.3` contract stays green — tokenless direct loopback is denied and the existing dashboard auth suites pass unmodified. (2026-08-05 ✅: `3001d6d5` — C6.1/C6.3 guards green unmodified)
- [x] **W21.5** Assert a workflow runs `make verify-deployment`, on the daily cron **and** on `refs/tags/v*` (**xfail → W23**, C14.1). (2026-08-05 ✅: `3001d6d5` — cron+tag verify-deployment xfails; 2026-08-06 ✅: `ca0c1d3f` — un-xfail after W23 by test-creator)
- [x] **W21.6** Assert exit code **2 (`driver_unavailable`) is a failure on the tag path** and tolerated on the cron (**xfail → W23**, D52). (2026-08-05 ✅: `3001d6d5` — D52 exit-2 policy xfails; 2026-08-06 ✅: `ca0c1d3f` — un-xfail after W23; helper restricted to verify step)
- [x] **W21.7** Assert the new drivers exist and are registered in `DRIVERS` (`scripts/verify_deployment.py:954-958`): authenticated proxy round-trip, volume upgrade/migration, multi-arch browser/GUI boot, cancellation cleanup (**xfail → W23**, C14.2). (2026-08-05 ✅: `3001d6d5` — four new DRIVERS xfails; 2026-08-06 ✅: `ca0c1d3f` — un-xfail; IDs reconciled to landed `DRIVERS` keys incl. `sandbox-scoped-token`)
- [x] **W21.8** Assert captured evidence is attached to the release (**xfail → W23**, C14.3). (2026-08-05 ✅: `3001d6d5` — evidence/verify attach xfail; 2026-08-06 ✅: `ca0c1d3f` — un-xfail after W23)
- [x] **W21.9** Author `docs/test-plans/prod-ready-batch-f.md`; `make lint typecheck`; commit + push. (2026-08-05 ✅: `3001d6d5` — test-plan + lint/typecheck + push)
...
- [x] **W22.1** **C6.2** — refuse `local_open_trust_address` (`src/sevn/ui/dashboard/services/auth.py`) whenever a tunnel or reverse proxy is configured. The existing tunnel mitigation forces `local_open=false`; the escape hatch must be subject to the same rule, not an exception to it. (2026-08-05 ✅: `apply_tunnel_local_open_policy` clears trust-address under tunnel + non-loopback bind)
- [x] **W22.2** Log a **loud boot warning** when the key is enabled, mirroring the proxy's unauthenticated-mode warning. (2026-08-05 ✅: `log_local_open_trust_address_boot_warning` + gateway lifespan call)
- [x] **W22.3** Decide the naming question the source document raises: either rename the key so it reads as dangerous, or record why the current name plus the boot warning is sufficient. A rename is a **breaking config change** — if taken, gate it behind schema migration and call it out in the PR body. Record the decision either way. (2026-08-05 ✅: **keep** `local_open_trust_address` — `trust_address` already signals address-based trust; refusal + boot warning + schema/Field "dangerous escape hatch" copy satisfy C6.2 without a breaking rename/migration)
- [x] **W22.4** **C6.4** — fix `src/sevn/cli/commands/dashboard_cmd.py`, which still tells the operator "loopback access — no login required" although `C6.1` made a token mandatory. Describe the actual flow. (2026-08-05 ✅: CLI states boot `dashboard-local-token` is required and appended when available)
- [x] **W22.5** Run **`make config-schema`**; update `docs/readmes/ui-mission-control.md`; add the Amendments to specs 24, 19 and 02 (D53). (2026-08-05 ✅: schema description + Amendments + readme)
- [x] **W22.6** Un-xfail W21.1–W21.3; `make lint typecheck`; commit + push — *(suggested: `fix(dashboard): guard the local-open trust-address escape hatch`)*. (2026-08-05 ✅: lint/typecheck/`ci-affected` green on `a4e660b6`; **un-xfail by test-creator** `66cf0ef4` — W21.1–W21.3 markers removed, 8 passed)
...
- [x] **W23.1** **C14.1** — add a job running **`make verify-deployment`** on the daily cron in `ci-supplementary.yml` and on `refs/tags/v*` in `ci-cd.yml`. The harness is 1,000+ lines with four working drivers and **zero** workflow references today. (2026-08-06 ✅: c432710b — `ci-supplementary.yml` cron job `verify-deployment` with `set +e` and exit-1 fail / exit-2 `::warning`; `ci-cd.yml` tag-path job `verify-deployment` needs `phase1+publish-ghcr+container-supply-chain`)
- [x] **W23.2** **D52** — treat exit **2 (`driver_unavailable`) as a failure on the release path** and as tolerated on the cron. A release that cannot run its own verification has not been verified. (2026-08-06 ✅: c432710b — cron: exit 2 → `::warning` and job succeeds; tag path: `make verify-deployment` exit 2 fails the job; `delivery-chain` aggregator requires `verify-deployment` for tag + dispatch)
- [x] **W23.3** **C14.2** — add drivers for the uncovered paths, registering each in `DRIVERS` (`scripts/verify_deployment.py:954-958`): **authenticated gateway→proxy round-trip** (the path Batch A fixes and nothing currently exercises), **volume upgrade/migration** from an existing operator volume, **multi-arch browser/GUI boot**, and **cancellation cleanup**. (2026-08-06 ✅: c432710b — 4 drivers `drive_authenticated_proxy_roundtrip`, `drive_volume_upgrade`, `drive_browser_gui_boot`, `drive_cancellation_cleanup` registered in `DRIVERS`; all print `VERIFY_OVERALL:`)
- [x] **W23.4** Add a driver or assertion for **sandbox-to-proxy scoped-token calls** once Batch E has merged; if E has not merged, record the gap and file it rather than asserting unimplemented behaviour. (2026-08-06 ✅: c432710b — Batch E (`3ce129f4`) IS merged in `origin/pre-0.0.1`; 5th driver `drive_sandbox_scoped_token` exercises `scope=sandbox` accept on `/web/*` + reject on `/llm/*` + `X-Sevn-Proxy-Token` service secret still accepted)
- [x] **W23.5** **C14.3** — attach captured evidence (`evidence/verify/`) to the release as an artifact, so "it works" is a downloadable artifact rather than an assertion in a PR body. (2026-08-06 ✅: c432710b — `actions/upload-artifact` uploads `evidence/verify/` as `deployment-verification-<sha>` with `if-no-files-found: error`; phase6 downloads it and bundles `evidence/verify/*.json` alongside SBOMs into the draft release)
- [x] **W23.6** Coordinate with D/W16.4 so the `HostConfig` limits check is wired **once**, not twice. (2026-08-06 ✅: c432710b — `drive_browser_gui_boot` asserts the resolved gateway image flips to `Dockerfile.gateway.browser` / `.gui`; the limits invariant is W16.4's responsibility — W23 does not duplicate it; documented in the W23 amendment to spec 25)
- [x] **W23.7** Update `25-cicd-full.md` Workflow matrix and Failure Modes in place (D53); un-xfail W21.5–W21.8; `make lint typecheck`; commit + push — *(suggested: `fix(ci): run deployment verification on cron and release tags`)*. (2026-08-06 ✅: c432710b — spec 25 Workflow matrix, `ci-cd.yml` trigger matrix, Artefacts table, Failure Modes, and a new W23 amendment section; `make lint typecheck ci-affected SEVN_CI_BASE=origin/pre-0.0.1` all green; W21.5–W21.8 un-xfail is `test-creator` reconciliation — deferred to a follow-up dispatch per D4)
...
- [x] **F-Verify** (`wave-verifier`): **`make verify-runtime`** and **`make verify-deployment`** — paste each `VERIFY_OVERALL:`. Runtime proof: with a tunnel configured, `local_open_trust_address: true` does **not** grant tokenless access; the boot warning appears; a normal `sevn dashboard` flow is not locked out; each new driver runs and reports. Confirm the landed `C6.1` denial still holds. (2026-08-07 ✅: `f2d04007` — VERIFY_OVERALL pass on all 9 drivers; F-V5 un-xfail landed; all 14 W21 tests green; C6.1 unmodified suite 15/15; `make ci-affected` exit 0; `evidence/verify/*.json`)
- [x] **F-Final:** xfail sweep (0 xfails); `graphify update .`; **drift sweep in one commit** (D7) covering the `ui-mission-control` readme fingerprint and `make about-docs-extract` for specs 24/19/02/25; CHANGELOG entries for C6.2, C6.4 and C14.\* (flag any rename from W22.3 as **breaking**); `make ci-resume SEVN_CI_BASE=origin/pre-0.0.1` until all steps pass. (2026-08-07 ✅: `e5a5ee47` — re-dispatch after FF-V1 fix; all 41 ci-resume steps green; 50/50 xfail sweep (W9+W21+W22+C6.1) PASSED; FF-V1 (`a4ff5ac2`) restored the `Cleanup quarantine tags on failure` step in `container-supply-chain`; FF-V2 (`8293b61d`) added h2 CVE-2026-71554 to the security allowlist (fix already on `origin/pre-0.0.1` via PR #242); FF-V3 (`e5a5ee47`) refreshed the spec-25 fingerprint after FF-V1 modified `.github/workflows/ci-cd.yml`; verdict **`pass`**)
- [x] **F-Thermos:** thermo-nuclear review; **clean including `low`** (D31); **blocks the PR**. Pay specific attention to a verification job that passes without actually running the drivers — a green job that skips is worse than no job. **Record the base SHA** to `.ignorelocal/waves/prod-ready-f-thermos-base.sha` and **declare every file the review changed** (D29). (2026-08-07 ✅: `57821c96` — clean including low; 7 original findings all resolved; rebase intact; P2/P3/P5 clean)
- [x] **F-Reverify** (**fresh `wave-verifier`**): convention-11 detection with `<batch>` = `f`. Any Thermos edit inside `src/sevn/ui/dashboard/services/auth.py` is an **auth surface authored without a RED test**: confirm a test fails if the trust-address refusal is deleted (P4a). Re-run the F-Verify proofs and `make ci-affected SEVN_CI_BASE="$BASE"`. Hand findings back (D4). **Blocks F-PR** (D29, D30). (2026-08-07 ✅: `978ea105` — fresh verifier per D30; all 5 proofs re-confirmed; P4a holds for F-Thermos fixes; P2/P3/P5 clean)
