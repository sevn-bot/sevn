...
Runnable as a plain script with the system ``python3`` (no third-party imports) via
``python3 scripts/changelog_validate.py``.
"""
...
def _default_rules_path() -> Path:
    """Return the bundled ``infra/changelog-rules.toml`` (repo root)."""
    return Path(__file__).resolve().parents[1] / "infra" / "changelog-rules.toml"
...
def lint_entries(
    ...
    for category in version["categories"]:
        for row in category["lines"]:
            ...
            if entry_rules.get("require_sentence_case", True):
                first = body[0]
                if not body.startswith("`") and first.isalpha() and first.islower():
                    errors.append(
