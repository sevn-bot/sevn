...
- [2026-07-24] Telegram ``/config`` → My sevn bot gains an owner-only persistent-tunnel toggle: "Turn tunnel on" stamps ``infrastructure.tunnel.autostart`` in ``sevn.json`` and starts the configured provider now, and the gateway re-launches it on every boot so the public tunnel survives a host restart alongside the gateway daemon; "Turn tunnel off" clears the flag and stops the provider. The button only appears when a runnable tunnel mode is configured (``sevn tunnel setup``), and the on-toast warns when the gateway daemon is not installed (the only case where boot-restart survival does not apply)
