...
- [2026-07-24] Telegram ``/config`` → My sevn bot gains an owner-only persistent-tunnel toggle: "Turn tunnel on" starts the configured provider and, only once it comes up healthy, stamps ``infrastructure.tunnel.autostart`` in ``sevn.json`` so the gateway re-launches it on every boot — the public tunnel survives a host restart alongside the gateway daemon; "Turn tunnel off" clears the flag and stops the provider. A failed start reports the error and persists nothing (so boot never retries a broken config). Boot re-launch is time-bounded and best-effort, so a missing ``cloudflared`` binary, an unresolved secret, or a hung provider CLI is skipped rather than stalling gateway startup. The button only appears when a runnable tunnel mode is configured (``sevn tunnel setup``), and the on-toast warns when the gateway daemon is not installed (the only case where boot-restart survival does not apply)
