...
=======
=======
- [2026-07-31] Draft-only Reddit karma loop skill (#74): bundled `reddit-karma-loop` discovers threads via the existing browser Reddit path, applies quality gates, drafts wiki/second-brain-grounded comments, enforces per-day caps and cooldowns, logs every decision to `.sevn/reddit-karma-loop/decisions.jsonl`, and requires `--confirm` before any post (`auto_post` not implemented; D11)
>>>>>>> e3f4161d (feat(skills): add draft-only Reddit karma loop skill)
- [2026-07-31] Hermes 4 model eval skill (#91): bundled `hermes-model-eval` runs a fixed golden_llm suite (tool-call, coding, summarization, policy/approval) with tokenless replay reports and opt-in live cross-model comparison via `SEVN_HERMES_MODEL_EVAL=1`; Hermes aliases are provider-configurable; results are advisory only (D9); W32 D21 records golden_llm-only path (no spy_hermes tooling)
>>>>>>> 0994fddc (feat(skills): add cross-model eval skill for Hermes 4 comparison)
- [2026-07-31] Buzz integration via ACP (#72): ``sevn acp`` stdio runtime bridges ``session/prompt`` into the gateway turn loop with ``session/update`` streaming; ``BuzzChannelAdapter`` + bundled ``buzz`` skill resolve ``BUZZ_PRIVATE_KEY`` / ``BUZZ_RELAY_URL`` through the secrets chain; optional ``acp`` extra documents relay setup
>>>>>>> 22ef9c4d (feat(channels): add ACP runtime and Buzz integration)
- [2026-07-31] First-token latency (TTFT) instrumentation (#78): ``gateway.turn.ttft`` trace spans, per-stage ``ttft`` samples in Mission Control, structured ``gateway_turn_startup_timings`` logs for registry/triage contributors; opt-in ``gateway.defer_mcp_discovery`` (default off, D9) moves MCP subprocess discovery off the boot critical path without changing turn output; opt-in ``gateway.cache_session_registry`` reuses registry snapshots via the W15 fingerprint seam
>>>>>>> 91602ad7 (perf(gateway): instrument and reduce first-token latency)
- [2026-07-31] **Breaking:** MCP registry tools renamed from ``server.tool`` to ``mcp__server__tool`` (#90) — dispatch accepts both forms during transition; update prompts, traces, and Triager references
- [2026-07-31] MCP integration hardening (#90): per-profile ``mcp_disabled_servers`` overlays on workspace ``mcp_enabled``; OAuth tokens at ``oauth.mcp.<server_id>`` via secrets chain with race-hardened callback flow; structured MCP events in ``<workspace>/logs/mcp.log``; Mission Control ``catalog_presets`` for minimal safe default toolsets
- [2026-07-31] Browser auth persistence (#92): configurable `skills.browser.password_store` / `SEVN_BROWSER_PASSWORD_STORE` (`basic` default for CI/Docker, `system` for OS keychain), `sevn browser clear-auth` to reset saved profiles, `sevn.browser.persistence` session markers + ephemeral profiles, and `sevn.browser.redaction` for cookie/profile path scrubbing in tool outputs
