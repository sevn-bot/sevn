...
| Second Brain | Filesystem wiki vault with layout-aware roles (`legacy`: `raw/`, `wiki/`, `outputs/`; `para`: PARA Obsidian folders); not a sync daemon. | [`paths.py`](src/sevn/second_brain/paths.py), [spec-27-second-brain](spec-27-second-brain) |
| Vault layout | Second Brain path model: `second_brain.layout` is `"legacy"` (default, OKF `wiki/raw/outputs`) or `"para"` (Obsidian PARA folders). Resolved by `VaultLayout`. | [`VaultLayout`](src/sevn/second_brain/paths.py), [spec-27-second-brain §3.2](spec-27-second-brain) |
| PARA layout | Obsidian-native vault layout: `00_Inbox`, `10_Projects`, `20_Areas`, `30_Resources` (+ `_sources`/`_outputs`), `40_Archive`, `90_Templates`, vault-root `index.md`/`log.md`. Opt-in via `layout: "para"`. | [`SecondBrainParaConfig`](src/sevn/config/sections/features.py), [spec-27-second-brain §3.2](spec-27-second-brain) |
| Layout role | Semantic vault directory mapped by `VaultLayout.role_dir`: `capture`, `projects`, `areas`, `curated`, `archive`, `templates`, `sources`, `outputs`, `index_note`, `log_note`. | [`VaultLayout`](src/sevn/second_brain/paths.py), [spec-27-second-brain §3.2](spec-27-second-brain) |
