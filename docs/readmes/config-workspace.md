...
You can also declare **deny rules** in config — permanent blocks on named tools (optionally narrowed by command pattern, domain, or path) that still apply even when a session is otherwise permissive. Wake-word listening stays **off** until you opt in under `voice.activation` in `sevn.json`. Mission Control on loopback still needs the boot `dashboard-local-token` unless you set `dashboard.local_open_trust_address` to restore legacy tokenless trust-by-address (default **off**).
...
`sevn.json` is a single document validated against [`infra/sevn.schema.json`](../../infra/sevn.schema.json). Domain sections live as Pydantic models under [`src/sevn/config/sections/`](../../src/sevn/config/sections/) — e.g. [`gateway.py`](../../src/sevn/config/sections/gateway.py), [`channels.py`](../../src/sevn/config/sections/channels.py), [`agent.py`](../../src/sevn/config/sections/agent.py), [`security.py`](../../src/sevn/config/sections/security.py), [`dashboard.py`](../../src/sevn/config/sections/dashboard.py), [`config_archive.py`](../../src/sevn/config/sections/config_archive.py). CLI section helpers live in the [`cli/config_sections/`](../../src/sevn/cli/config_sections/) package ([`config_sections/__init__.py`](../../src/sevn/cli/config_sections/__init__.py)) and expose stable slugs for **`sevn config <slug>`** (for example `sevn config gateway`).

**`dashboard`** ([`DashboardWorkspaceConfig`](../../src/sevn/config/sections/dashboard.py); schema under `dashboard` in [`infra/sevn.schema.json`](../../infra/sevn.schema.json)): `enabled` mounts Mission Control; `local_open` (nullable) still gates the loopback login bypass when effective (default true on loopback-only bind with no tunnel; forced false when a tunnel is active). `local_open_trust_address` defaults **false** — loopback clients must present the boot `dashboard-local-token`; set **true** only to restore legacy tokenless direct-loopback access.
