...
When `gateway.session_mirror` is enabled, messages are also appended to workspace JSONL under `sessions/telegram/chats/`. For groups and forum topics, [`format_named_path_segment`](../../src/sevn/gateway/session/path_names.py#L68) builds `{sanitized_title}--{id}` folder segments (e.g. `My_Group--1001234567890/topics/General--7/`); non-topic group scopes use `…/general/`. Titles come from SQLite via [`SessionPathNameResolver`](../../src/sevn/gateway/session/path_names.py#L37), which looks up persisted group and forum topic names. Private chats and scopes where no title is known yet keep ID-only segments; existing JSONL on disk is not moved when a name is learned later. When a group title is first learned or renamed mid-session, subsequent mirror writes may land in a new folder while earlier lines remain under the prior path (D2: no retroactive moves).
