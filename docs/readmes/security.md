...
2. **Host user-namespace policy.** The kernel must permit unprivileged user namespaces. Ubuntu 23.10+ restricts them through `/proc/sys/kernel/apparmor_restrict_unprivileged_userns` (default `1` on 24.04). `scripts/check-browser-host.sh` (`make check-browser-host`, and automatically on `make compose-browser-up` / `make compose-gui-up`) **fails closed** when that sysctl is non-zero rather than letting `compose up` produce a browser that never starts. This is a property of the machine, not of the repo, which is why it runs on the deploy path and not in CI's static gates — `make check-compose-default` asserts only that the committed overlays pin the profile.
