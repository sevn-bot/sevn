...
def test_manifest_includes_para_layout_capability() -> None:
    manifest = load_manifest()
    ids = {cap.capability_id for cap in manifest.capabilities}
    assert "memory.second_brain.layout" in ids


def test_vault_path_capability_mentions_adoption_note() -> None:
    manifest = load_manifest()
    cap = next(
        c for c in manifest.capabilities if c.capability_id == "memory.second_brain.vault_path"
    )
    assert cap.description is not None
    assert "adopt" in cap.description.lower() or "detect" in cap.description.lower()
