...
- `gateway.token` — `e2e-dev-token` for Playwright `/login` journey against compose.
- `channels.webchat` — JWT + `allowed_origins` for localhost compose (`13001`) and
  Playwright webServer (`13002`).
...
`tests/e2e/dispatch-turn.spec.ts` receives deterministic echo replies.
