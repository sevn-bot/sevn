...
TWEXAPI_SECRET_ALIAS = "SEVN_SECRET_TWEXAPI"
...
class TwexApiSettings:
    ...
    Attributes:
        enabled (bool): When false, TwexAPI medium is disabled.
...
def load_twexapi_settings(
    ...
    base_url = (
        base_raw.strip().rstrip("/")
        if isinstance(base_raw, str) and base_raw.strip()
        else DEFAULT_TWEXAPI_BASE_URL
    )
    enabled = twex_block.get("enabled", block.get("twexapi_enabled", True))
    return (
        TwexApiSettings(
