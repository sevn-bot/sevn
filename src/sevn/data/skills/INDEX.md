...
| discogs-collection | Discogs user collection — folders, items, value, and collection search (opt-in group; expanded in W5). |
| discogs-database | Discogs public catalog — search, artist/release/master/label lookups, price suggestions, marketplace stats. |
| discogs-identity | Discogs authed user profile, lists, contributions, and whoami smoke-test (opt-in group; expanded in W7). |
| discogs-marketplace | Discogs marketplace — inventory, listings, orders, and fees (opt-in group; expanded in W4). |
| discogs-wantlist | Discogs user wantlist search and CRUD (opt-in group; expanded in W6). |
