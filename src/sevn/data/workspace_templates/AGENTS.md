...
- **§4 Web** — `serp` (no key, prefer), `web_search` (Brave key), `get_page_content`, `web_fetch`; **`last30days`** (multi-source social research — run engine via skill, not web-only); native **`browser`** tool (CDP engine), **`browser-harness`**; `integration_call` for other connectors
