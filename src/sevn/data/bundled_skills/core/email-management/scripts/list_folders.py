#!/usr/bin/env python3
"""Bundled ``email-management`` skill — list IMAP folders for one account.

Module: sevn.data.bundled_skills.core.email-management.scripts.list_folders
...
def main(argv: list[str] | None = None) -> int:
    """List mailbox folders for one configured account.
    ...
    if account.backend == "gmail_api" or dry_run_requested(cli_flag=args.dry_run):
        write_ok(gmail_api_plan(account, operation="list_labels"))
        return 0

    try:
        password = resolve_password(account)
        folders = list_imap_folders(account, password)
    except ValueError as exc:
        write_error(code="VALIDATION_ERROR", error=str(exc))
        return 1
