#!/usr/bin/env python3
"""Bundled ``discogs-marketplace`` skill — delete a marketplace listing.

Module: sevn.data.bundled_skills.core.discogs-marketplace.scripts.delete_listing
...
def _would_do(args: argparse.Namespace) -> dict[str, Any]:
    return {"action": "delete_listing", "listing_id": args.listing_id}


def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    client.listing(args.listing_id).delete()
    payload = write_ok({"deleted": _would_do(args)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run delete-listing CLI.
