#!/usr/bin/env python3
"""Bundled ``discogs-marketplace`` skill — fetch one marketplace listing.

Module: sevn.data.bundled_skills.core.discogs-marketplace.scripts.get_listing
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--id", required=True, help="Discogs listing id")
    return parser


def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    listing = client.listing(int(args.id))
    payload = write_ok({"listing": serialize_object(listing)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run listing lookup CLI.
