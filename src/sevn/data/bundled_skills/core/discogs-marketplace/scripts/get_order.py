#!/usr/bin/env python3
"""Bundled ``discogs-marketplace`` skill — fetch one marketplace order.

Module: sevn.data.bundled_skills.core.discogs-marketplace.scripts.get_order
...
def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    order = client.order(int(args.id))
    payload = write_ok({"order": serialize_object(order)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run order lookup CLI.
