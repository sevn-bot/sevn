#!/usr/bin/env python3
"""Bundled ``lume`` skill — stop a VM via ``lume stop``.

Module: sevn.data.bundled_skills.core.lume.scripts.stop
...
def main(argv: list[str] | None = None) -> int:
    """Run ``lume stop`` for the named VM.
    ...
    parser.add_argument("--name", required=True, help="Name of the VM to stop.")
    args = parser.parse_args(argv)

    try:
        cfg = ensure_lume_ready()
        proc = run_lume_cli(cfg, ["stop", args.name])
