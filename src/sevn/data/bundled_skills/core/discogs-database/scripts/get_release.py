#!/usr/bin/env python3
"""Bundled ``discogs-database`` skill — fetch one release by id.

Module: sevn.data.bundled_skills.core.discogs-database.scripts.get_release
...
def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    ...
    payload = write_ok({"release": serialize_object(release)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run release lookup CLI.
