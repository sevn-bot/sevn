#!/usr/bin/env python3
"""Bundled ``discogs-database`` skill — release marketplace stats.

Module: sevn.data.bundled_skills.core.discogs-database.scripts.marketplace_stats
    ...
    payload = write_ok(
        {
            ...
            "marketplace_stats": serialize_object(stats),
        }
    )
    return 0, payload
...
def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run release marketplace-stats CLI.
