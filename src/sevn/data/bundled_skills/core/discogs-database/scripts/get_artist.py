#!/usr/bin/env python3
"""Bundled ``discogs-database`` skill — fetch one artist by id.

Module: sevn.data.bundled_skills.core.discogs-database.scripts.get_artist
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--id", required=True, help="Discogs artist id")
    return parser


def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    artist = client.artist(int(args.id))
    payload = write_ok({"artist": serialize_object(artist)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run artist lookup CLI.
