#!/usr/bin/env python3
"""Bundled ``discogs-database`` skill — release price suggestions.

Module: sevn.data.bundled_skills.core.discogs-database.scripts.price_suggestions
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--id", required=True, help="Discogs release id")
    return parser


def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    release = client.release(int(args.id))
    suggestions = release.price_suggestions
    payload = write_ok(
        {
            "release_id": int(args.id),
            "price_suggestions": serialize_object(suggestions),
        }
    )
    return 0, payload
...
def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run release price-suggestions CLI.
