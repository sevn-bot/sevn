#!/usr/bin/env python3
"""Bundled ``discogs-database`` skill — fetch one label by id.

Module: sevn.data.bundled_skills.core.discogs-database.scripts.get_label
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--id", required=True, help="Discogs label id")
    return parser


def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    label = client.label(int(args.id))
    payload = write_ok({"label": serialize_object(label)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run label lookup CLI.
