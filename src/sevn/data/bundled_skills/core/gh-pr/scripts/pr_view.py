#!/usr/bin/env python3
"""Bundled ``gh-pr`` skill — view pull request.

Module: sevn.data.bundled_skills.core.gh-pr.scripts.pr_view
...
def main(argv: list[str] | None = None) -> int:
    """Run gh pr view pull request CLI.
    ...
    try:
        payload = asyncio.run(
            gh_pr.view_pull_request(hooks, repo=args.repo, pull_number=args.pr_number)
        )
    except (RuntimeError, ValueError) as exc:
        write_error(code="GITHUB_PR_VIEW_FAILED", error=str(exc))
        return 1
    write_ok(payload)
    return 0
