#!/usr/bin/env python3
"""Bundled ``discogs-collection`` skill — move a collection item to another folder.

Module: sevn.data.bundled_skills.core.discogs-collection.scripts.move_release
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--target-folder-id", required=True, type=int, help="Destination folder id")
...
def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    ...
    folder.move_release(instance, args.target_folder_id)
    payload = write_ok({"moved": _would_do(args)})
    return 0, payload


def main(argv: list[str] | None = None) -> tuple[int, dict[str, Any]]:
    """Run move-release CLI.
