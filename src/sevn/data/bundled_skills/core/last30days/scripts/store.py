...
def get_findings_new_in_run(run_id: int) -> List[Dict[str, Any]]:
    """Return findings whose first-ever sighting for the topic is *run_id*.

    URL re-sightings update aggregate rows but stay out of daily delivery.
    """
    conn = _connect()
    try:
        rows = conn.execute(
            """SELECT f.* FROM findings f
               INNER JOIN (
                   SELECT finding_id, MIN(run_id) AS first_run_id
                   FROM finding_sightings
                   WHERE topic_id = (
                       SELECT topic_id FROM research_runs WHERE id = ?
                   )
                   GROUP BY finding_id
               ) first_sighting
                 ON first_sighting.finding_id = f.id
                AND first_sighting.first_run_id = ?
               WHERE f.dismissed = 0
               ORDER BY f.engagement_score DESC, f.first_seen DESC""",
            (run_id, run_id),
        ).fetchall()
        return [dict(r) for r in rows]
    finally:
