...
def _emit(result: dict) -> int:
    ...
    if status in {"no_topics", "no_enabled", "not_found"}:
        return _fail(code=status.upper(), message=str(result.get("message", status)))
    write_ok(result)
    return 0
...
def generate_daily(since: str = None) -> dict:
    ...
    result = {
        ...
        "top_finding": {
            ...
            "url": top_overall.get("source_url", ""),
            "author": top_overall.get("author", ""),
