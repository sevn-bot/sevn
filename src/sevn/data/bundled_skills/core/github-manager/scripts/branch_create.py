#!/usr/bin/env python3
"""Bundled ``github-manager`` skill — create branch.

Module: sevn.data.bundled_skills.core.github-manager.scripts.branch_create
...
def main(argv: list[str] | None = None) -> int:
    """Run github manager create branch CLI.
    ...
    try:
        payload = asyncio.run(
            github_manager.create_branch(
                hooks, repo=args.repo, branch=args.branch, from_ref=args.from_ref
            )
        )
    except (RuntimeError, ValueError) as exc:
        write_error(code="GITHUB_BRANCH_CREATE_FAILED", error=str(exc))
        return 1
    write_ok(payload)
    return 0
