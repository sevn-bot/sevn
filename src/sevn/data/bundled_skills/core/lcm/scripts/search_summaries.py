#!/usr/bin/env python3
"""Bundled ``lcm`` skill — session-end summary search.

Module: sevn.data.bundled_skills.core.lcm.scripts.search_summaries
...
from sevn.lcm.query import search_summaries_scoped
from sevn.lcm.script_cli import open_workspace_db, session_key_from, write_error, write_ok


def main() -> int:
    """Search ``lcm_summaries`` session-end rows by keyword.
    ...
    Examples:
        ...
        ...     "summary_kind, created_at) VALUES ('sess1', ?, 'Discussed caching', 0, "
        ...
        >>> argv = sys.argv
        >>> try:
        ...     sys.argv = ["search_summaries", "--query", "caching", "--scope", "workspace"]
