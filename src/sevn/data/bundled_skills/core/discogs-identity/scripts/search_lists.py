#!/usr/bin/env python3
"""Bundled ``discogs-identity`` skill — search a user's lists by name.

Module: sevn.data.bundled_skills.core.discogs-identity.scripts.search_lists
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--name", help="Filter lists by name (case-insensitive substring)")
    parser.add_argument("--page", type=int, default=1)
    parser.add_argument("--per-page", type=int, default=50)
    return parser


def _list_name(item: dict[str, Any]) -> str:
...
def _worker(args: argparse.Namespace, client: Any) -> tuple[int, dict[str, Any]]:
    ...
    if args.name:
        ...
        items = [item for item in items if needle in _list_name(item).casefold()]
