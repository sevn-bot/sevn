#!/usr/bin/env python3
"""Bundled ``proton-management`` skill — list Pass items (metadata)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    cmd = ["pass", "items", "list", "--output", args.output]
    if args.vault:
    ...
    if args.output == "json":
        try:
            ...
            data = {"raw": out}
        write_ok({"items": data, "count": len(data) if isinstance(data, list) else None})
    else:
        write_ok({"text": out})
    return 0
