"""Shared serialization and CLI helpers for discogs-wantlist scripts."""
...
def serialize_object(obj: object) -> dict[str, Any]:
    ...
    if hasattr(obj, "release") and not callable(obj.release):
        release = obj.release
        if release is not None:
            data["release"] = serialize_object(release)
    if hasattr(obj, "basic_information") and not callable(obj.basic_information):
        info = obj.basic_information
        if info is not None:
            data["basic_information"] = serialize_object(info)
    return data
