...
Module: sevn.gateway.user_model_turn
Depends: sevn.agent.providers.resolve, sevn.config.model_resolution, sevn.gateway.post_turn_hooks,
    sevn.gateway.turn_metadata, sevn.memory.user_model
...
from sevn.gateway.turn_metadata import load_turn_metadata
...
def lookup_user_text_for_turn(
    ...
    *,
    ...
    Examples:
        ...
        >>> from sevn.gateway.user_model_turn import lookup_user_text_for_turn
...
def _owner_lane_allowed(ctx: PostTurnContext) -> bool:
    ...
    Examples:
        ...
        >>> from sevn.gateway.post_turn_hooks import PostTurnContext
        >>> from sevn.gateway.user_model_turn import _owner_lane_allowed
        >>> from sevn.storage.migrate import apply_migrations
        >>> db = sqlite3.connect(":memory:")
        >>> apply_migrations(db)
        >>> _ = db.execute(
...
def _merge_action(
    ...
    *,
    ...
    Examples:
        ...
        >>> from sevn.memory.user_model.models import InferredFact, UserProfile
        >>> from sevn.gateway.user_model_turn import _merge_action
...
async def _emit_user_model_span(
    ...
    *,
    ...
    Examples:
        >>> import asyncio
        >>> from sevn.agent.tracing.sink import NullTraceSink
        >>> from sevn.gateway.user_model_turn import _emit_user_model_span
        >>> asyncio.run(
